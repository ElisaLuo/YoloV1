...
def plot_image(image, boxes, fig_num):
    ...
    for box in boxes:
        ...
        confidence = str("{:.3f}".format(box[1]))
        ...
        ax.text((upper_left_x+box[2]) * width, 
        (upper_left_y + box[3]) * height, 
        label + " " + confidence,
        backgroundcolor='k',
        color='w',
        fontsize = 'xx-small',
    ...
    plt.savefig('shapesResultsV6/'+str(fig_num))
    #plt.show()
    plt.close()
    print("saving "+fig_num)
...
def get_bboxes(
    ...
    for batch_idx, (x, labels, z) in enumerate(loader):
        x = x.to(device)
